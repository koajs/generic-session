...
module.exports = function (options) {
  ...
  const EMPTY_SESSION_HASH_NOT_SECURE = hash(generateSession({ secure: false }));
  const EMPTY_SESSION_HASH_SECURE = hash(generateSession({ secure: true }));
  ...
  function generateSession(ctx) {
    ...
    if (!session.cookie.hasOwnProperty('secure')) {
      // auto set secure on https request
      session.cookie.secure = ctx.secure;
    }
  ...
  function *refreshSession (session, originalHash, isNew) {
    ...
    if (!options.allowEmpty && isNew &&
        (newHash === EMPTY_SESSION_HASH_NOT_SECURE || newHash === EMPTY_SESSION_HASH_SECURE)) {
  ...
  function *session(next) {
    ...
    this.regenerateSession = function *regenerateSession() {
      ...
      this.session = generateSession(this);
      this.sessionId = genSid.call(this, 24);
      sessionIdStore.reset.call(this);
  ...
  function *deferSession(next) {
    ...
    this.regenerateSession = function *regenerateSession() {
      ...
      this._session = generateSession(this);
      this.sessionId = genSid.call(this, 24);
      sessionIdStore.reset.call(this);
      debug('created new session: %s', this.sessionId);
      isNew = true;
