 ...
 *     you should `await this.session` to get the session if defer is true, default is false
...
module.exports = function (options) {
   ...
   */
  async function refreshSession(session, originalHash, isNew) {
    ...
    if (!session) {
      if (!isNew) {
        ...
        sessionIdStore.reset.call(this);
        return await store.destroy(this.sessionId);
      }
    ...
    if(this._sessionSave === true) {
      ...
      return await saveNow.call(this, this.sessionId, session);
    }
    ...
    await saveNow.call(this, this.sessionId, session);

  }

  async function saveNow(id, session) {
    compatMaxage(session.cookie);
   ...
   * ```
   */
  async function session(next) {
    this.sessionStore = store;
    if (this.session || this._session) {
      return await next;
    }
    let result = await getSession.call(this);
    if (!result) {
      return await next;
    }
    ...
    this.regenerateSession = async function regenerateSession() {
    ...
    try {
      await refreshSession.call(this, this.session, result.originalHash, result.isNew);
    } catch (err) {
   ...
   * ```
   */
  async function deferSession(next) {
    this.sessionStore = store;
    ...
    this.__defineGetter__('session', async function () {
      if (touchSession) {
        return this._session;
      }
      touchSession = true;
    ...
    if (touchSession) {
      ...
      await refreshSession.call(this, this._session, originalHash, isNew);
    }
  }
};
